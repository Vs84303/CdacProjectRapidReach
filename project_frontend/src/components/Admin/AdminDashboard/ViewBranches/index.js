  ...
  return (
    <div>
      <div className='container my-5'>
        <h3 className='utext my-5'>RapidReach Branches</h3>
        <button className='btn btn-info mb-3' onClick={() => { setAction('add'); setNewBranch({ branchName: '', phone: '', email: '', addressId: { address: '', address2: '', postalCode: '', city: '', state: '', country: '' } }); openModal(); }}>Add a Branch</button>
                      ...
                      <div className="card-body p-0">
                        <div className="table-responsive table-scroll" data-mdb-perfect-scrollbar="true" style={{ position: 'relative', height: '700px' }}>
                          <table className="table table-striped mb-0">
                            <thead style={{ backgroundColor: '#31D2F2' }}>
                              <tr>
                            ...
                            <tbody>
                              {branches.map(branch => (
                                <tr key={branch.branchId}>
                                  ...
                                  <td><button type='button' className='btn btn-outline-info' onClick={() => { viewEmployees(branch.branchId) }}>View Employees</button></td>
                                  <td><button type='button' className='btn btn-outline-info' onClick={() => { setAction('update'); updateBranch(branch.branchId) }}>Update Branch</button></td>
                                  <td><button type='button' className='btn btn-outline-info mx-3' onClick={() => { deleteBranch(branch.branchId) }}>Delete Branch</button></td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
          ...
          <div className='modal-content'>
            ...
            <form>
              ...
              <div className='modal-footer'>
                <button type='button' className='btn btn-outline-info' onClick={closeModal}>Cancel</button>
                {action === 'add' ? <button type='submit' className='btn btn-info' onClick={addBranch}>Add Branch</button> : <button type='submit' className='btn btn-info' onClick={() => {confirmBranchUpdate(newBranch.branchId)}}>Confirm Branch Update</button>}
              </div>
            </form>
          </div>
        </div>
